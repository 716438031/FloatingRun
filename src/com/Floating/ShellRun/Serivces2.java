...
public class Serivces2 extends Service {
	...
	private void run() {
		...
		if (out) {
			if (!result.successMsg.equals("")) {
				Toast.makeText(Serivces2.this, result.successMsg,
						Toast.LENGTH_SHORT).show();
				// 成功的提示
			}
			if (!result.errorMsg.equals("")) {
				Toast.makeText(Serivces2.this, result.errorMsg,
						Toast.LENGTH_SHORT).show();
				// 错误的提示
			}
			if (result.successMsg.equals("") && result.errorMsg.equals("")) {
				Toast.makeText(Serivces2.this, "没有返回值",
						Toast.LENGTH_SHORT).show();
			}
		}
	}
